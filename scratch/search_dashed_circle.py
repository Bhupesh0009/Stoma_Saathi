...
out_path = r"c:\Users\Bhupesh\Desktop\Stoma Saathi\scratch\recovered_dashed_circle.txt"

with open(transcript_path, "r", encoding="utf-8") as f, open(out_path, "w", encoding="utf-8") as out:
    for line in f:
        item = json.loads(line)
        content = item.get("content", "")
        if "_DashedCirclePainter" in content:
            out.write(f"--- Step {item.get('step_index')} ({item.get('type')}) ---\n")
            lines = content.splitlines()
            for idx, l in enumerate(lines):
                if "_DashedCirclePainter" in l and ("class" in l or "const" in l):
                    start = max(0, idx - 2)
                    end = min(len(lines), idx + 25)
                    for k in range(start, end):
                        out.write(f"{k}: {lines[k]}\n")
                    out.write("-" * 40 + "\n")
print("Done writing recovered dashed circle.")
